# Paper Monitor 中文说明

[English](README.md) · [Windows 安装与打包说明](README_WINDOWS.md)

Paper Monitor 是一个仅支持 Windows、本地优先的桌面文献监控工具。它从 Crossref、RSS、可选的 arXiv 和可选的 OpenAlex 检索论文，按照明确选择的期刊范围与研究关键词进行筛选，并把所有结果统一写入同一个本地 SQLite 文献生命周期。

默认检索方向覆盖全固态电池、硫化物和卤化物固态电解质、LATP、LLZO/LLZTO、硅负极与钠电池。内置的 300 本跨学科期刊目录覆盖 AI、计算机、工程、医学、生命科学、物理、化学、材料、环境、数学和社会科学。用户仍可自由修改关键词、期刊范围和检索方向。软件不依赖云端后台或大模型服务，也不会上传阅读记录。

## 当前架构

```mermaid
flowchart LR
    Trigger["主窗口 / 原生托盘 / Windows 任务计划"] --> Refresh["RefreshExecution"]
    Refresh --> Sources["Crossref / RSS / arXiv / OpenAlex"]
    Sources --> Match["期刊范围与检索词筛选"]
    Match --> Lifecycle["ArticleLifecycle · SQLite"]
    Lifecycle --> Toast["Windows Toast 适配器"]
    Lifecycle --> Snapshot["Dashboard 快照"]
    Snapshot --> UI["pywebview 主窗口"]
```

1. 定时任务、托盘操作和主界面刷新全部进入同一个有明确边界的 `RefreshExecution`。它负责一轮刷新中的全局互斥、来源检索、筛选、持久化和按刷新意图发送通知。
2. 来源检索使用有界并发、超时、重试和受容量限制的 Crossref 磁盘缓存。单个来源失败时保留其他成功来源的结果，并记录来源级诊断。
3. `ArticleLifecycle` 是文章身份、首次检测时间、界面展示、通知接受状态、刷新诊断和保留期限的唯一所有者；这些状态以事务方式写入同一个 SQLite 数据库。
4. Dashboard 直接读取本地生命周期快照，不会因为打开页面再次联网。后台刷新完成后会使旧的内存快照失效，因此通知中出现的新论文会在主界面下一次读取时同步出现。
5. 首页按首次检测时间倒序排列并分组，让刚检索到的论文保留在第一页；来源发表日期单独显示并保留原始精度。活动记录在 30 天后永久删除。

Windows 任务计划程序只在检索到期时唤醒短时工作进程，完成后立即退出。关闭主窗口也会释放 Python、WebView 和本地桥接服务。用户可以选择保留一个轻量原生 C 托盘；它不持有检索或数据库状态。“登录 Windows 时启动”使用独立任务，只静默启动托盘，不打开窗口，也不会立即联网。

## 主要功能

### 1. 桌面应用

Windows 版本使用一个原生窗口展示 Dashboard 和设置页面；重复启动会切换并聚焦已有窗口，不会创建多个相互竞争的窗口。

Windows 版关闭窗口后会结束本次界面进程并释放 WebView、Python 和本地服务占用，只保留用户选择启用的轻量原生 C 托盘。托盘本身不包含网络检索、数据库或页面渲染逻辑。开启“后台监控”后，由 Windows 任务计划程序在检索到期时启动一个短时刷新进程；完成检索、存储和通知后进程立即退出，因此两次检索之间不会常驻 Python、WebView 或本地 HTTP 服务。

Windows 新文章通知按本地日期折叠，每篇只显示文章名和较小字号的期刊名；点击对应通知会使用默认浏览器打开文章网页。

常用操作包括：

- 手动检索最新文献。
- 打开 Dashboard 查看匹配结果。
- 打开设置并调整检索范围。
- 发送测试通知，确认 Windows 通知权限正常。

### 2. 文献检索

Paper Monitor 使用 Crossref、RSS、可选的 arXiv 和可选的 OpenAlex 进行文献检索。OpenAlex 默认关闭，启用时必须配置 API Key；公开默认配置不要求用户提供密钥。arXiv 属于预发表来源，默认不勾选，需要用户在期刊筛选中手动启用。

Crossref 支持按期刊有界并发请求、分页、超时、重试和磁盘缓存；缓存上限为 64 MiB / 512 个文件。单个来源暂时失败不会丢弃其他来源已经成功返回的论文。

检索范围可以通过以下方式控制：

- 在期刊列表中明确勾选或取消特定期刊。
- 手动启用 arXiv 预发表来源。
- 直接选择硫化物固态电解质、卤化物固态电解质、LATP、LLZO/LLZTO、硅负极或钠电池检索方向。
- 修改检索词和查询语句。
- 设置 1 天或 2 天刷新间隔，以及确定的本地 `Start Time`；默认每天 `09:00`。
- 设置排除词，过滤明显无关的结果。

期刊目录和影响力信息来自随版本冻结的 `journal_metrics.json`。统一显示的 `2Y Impact` 取自 OpenAlex 的 two-year mean citedness；它是开放的两年引用影响指标，不是 Clarivate Journal Impact Factor。默认配置文件是 `config.example.json`。

### 3. 本地生命周期与去重

软件会把已检索到的文章统一记录到本地 SQLite 数据库。定时刷新、托盘刷新和主界面刷新共用同一份状态；已经在软件中展示过、已被 Windows 接受通知或交付结果不确定的论文不会重复通知，只有明确被系统拒绝的通知才允许重试。通知状态和 Dashboard 文章来自同一次事务提交，不存在第二套后台结果库。活动列表只保留最近 30 天的数据，过期记录会被直接删除，不保留隐藏历史。

本地运行数据默认保存在：

```text
%APPDATA%\PaperMonitor
```

这些运行数据不会上传到 GitHub，也不会上传到外部服务器。

### 4. Dashboard

Dashboard 由本地应用生成，用来查看已经保存的检索结果和分析结果；打开页面本身不会自动触发新的网络检索。

Dashboard 支持：

- 按首次检测时间倒序和分组显示文章，确保后台刚发现的论文出现在第一页。
- 显示论文标题、作者、期刊、本地影响力参考和 URL，不在主页显示摘要。
- 按时间、影响因子或相关性排序。
- 分别显示 `Detected` 和 `Published` 日期。
- 对只提供年月的来源保留原始日期精度，不虚构具体日期。
- 点击论文标题跳转到官方页面查看摘要和全文信息。

当按影响因子排序时，文章会按期刊影响因子排列，不再按日期分栏。

### 5. Keyword Analysis

Keyword Analysis 用于统计指定时间范围内的研究热点。它会根据日期范围和期刊范围重新检索文献，并基于标题进行快速分析。

主要功能包括：

- 选择起止日期。
- 手动勾选用于分析的期刊。
- 选择快速分析或更完整的分析模式。
- 自动提取候选关键词。
- 使用屏蔽词过滤通用词或干扰词。
- 编辑自定义分类词库。
- 查看不同分类的占比和文章数量。
- 展开分析文章列表，查看用于统计的论文标题、DOI、期刊和作者。

该功能适合做阶段性热点判断，例如统计某一年或某几个月内，固态电解质、硫化物、氧化物、卤化物、界面、电极等方向的大致占比。

## 下载和安装

打开 GitHub Release 页面：

```text
https://github.com/Dahe-Labs/paper-monitor/releases
```

最新 Release 只发布 Windows 下载文件：

```text
Paper-Monitor-Windows-x.y.z-Setup.exe
Paper-Monitor-Windows-x.y.z.zip
Paper-Monitor-Windows-x.y.z.exe
SHA256SUMS-x.y.z.txt
```

Windows 用户优先下载 `-Setup.exe` 安装包；安装器使用稳定 AppId，检测到旧版本时会执行升级，并默认保留 `%APPDATA%\PaperMonitor` 中的配置和文献数据库。需要免安装运行时可下载 ZIP，独立 EXE 也会保留。Windows 发布流程会同时生成 SHA256 校验文件。

公开 Release 的可执行文件应使用可信 Authenticode 证书签名。GitHub Actions 生成的未签名 Windows-only 工件仅用于测试，Windows 可能显示“未知发布者”警告。

## 首次使用

1. 安装后从开始菜单打开 `Paper Monitor`。
2. 如果 Windows 请求通知权限，选择允许。
3. 运行一次测试通知，确认通知可以正常弹出。
4. 打开设置，检查默认检索范围和关键词。
5. 点击刷新，等待软件检索并生成 Dashboard。
6. 打开 Dashboard 查看匹配到的文献。

## 设置说明

### 检索设置

这里可以调整：

- 期刊范围：直接维护需要检索的明确期刊列表，不再使用 Top N 自动选择。
- 刷新频率：控制后台计划任务多久唤醒一次进行检索。
- 文献检索方向：选择或修改当前研究方向的检索语句。

修改设置后，点击窗口右上角的 `Apply` 生效。没有修改时，`Apply` 会保持不可点击；修改后会变亮，点击后窗口不会关闭，用户可以继续调整其他设置。

### 后台监控

Windows 设置页中的“后台监控”不会把完整应用加入开机启动。启用后只会注册当前用户的 Windows 计划任务，到期时运行一次无窗口刷新并自动退出；关闭该选项会立即移除计划任务，已有配置、数据库和 Dashboard 历史不会被删除。升级安装还会清理旧版本使用的 `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\PaperMonitor` 常驻启动项。卸载时会先禁用计划任务、取消正在进行的刷新，再等待主窗口、托盘和刷新进程全部退出，避免卸载结束后仍有旧进程运行。

“登录 Windows 时启动”是另一个独立选项。启用后会注册单独的当前用户登录任务，只在登录后静默启动轻量原生托盘，不打开主窗口，也不会立即执行网络检索；Python 启动器完成托盘交接后随即退出。免安装版同样可以使用这两个计划任务，但任务会记录程序当前的绝对路径；移动解压目录后，需要从新位置打开一次软件以更新任务路径。

同一天的新论文通知会在 Windows 通知中心折叠为一组。展开后每项只显示最多两行的论文标题和较小字号的期刊名；点击通知会交给默认浏览器打开论文网页。通知使用 Paper Monitor 自己的 Windows 应用身份，不再归入 `Python`。

关键词分析控件只在首次打开分析页时初始化。论文较多时，Dashboard 先显示最近 50 篇，并通过“Show older papers”继续加载；全部论文仍保留在本地数据中。Crossref 缓存上限为 64 MiB/512 个文件，升级和卸载还会清理旧版本遗留的持久 WebView2 缓存，但不会删除用户配置和论文数据库。

### 检索词管理

默认检索词全部为英文。用户可以根据自己的研究方向自由修改。

适合添加的词包括：

- 材料体系，例如 `sulfide electrolyte`、`oxide electrolyte`、`halide electrolyte`。
- 器件方向，例如 `all-solid-state battery`、`lithium metal anode`。
- 机制方向，例如 `interfacial impedance`、`dendrite`。

排除词用于过滤无关结果，例如激光、照明、硬盘等和固态电池无关的语义。

### 期刊筛选

期刊页面支持：

- 手动勾选或取消具体期刊。
- 在底部单独启用 arXiv 预发表来源。
- 按名称、别名搜索，或按学科类别筛选。
- 按 `2Y Impact`、目录排名或名称排序显示。
- 使用 300 本跨学科期刊元数据，每本均显示统一的影响力标签。
- arXiv 会显示在候选列表中，并且只在用户明确启用后参与检索。

如果用户只想监控少数期刊，只需在期刊列表中保留需要的期刊。该明确选择列表同时用于日常刷新和 Keyword Analysis；单次分析可以缩小范围，但不能加入设置中未选择的期刊。

## 从源码构建

需要：

- Windows 10/11
- Python 3.12
- PowerShell
- Inno Setup 6 或 7

运行 Python 测试：

```bash
python -m unittest discover -s tests
```

构建完整 Windows 发布包：

`requirements-windows.txt` 是人工维护的顶层依赖范围；CI、发布以及可复现的本地 Windows 打包统一从 `requirements-windows.lock.txt` 安装。

```powershell
python -m pip install -r requirements-windows.lock.txt
.\scripts\package_windows_release.ps1 -Version 0.1.16
```

## 项目结构

```text
paper_monitor/          Python 检索、筛选、存储、Dashboard 和关键词分析逻辑
tests/                  Python 测试
scripts/                构建和安装脚本
windows/                Windows 入口、安装程序和图标资源
journal_metrics.json    期刊影响因子和元数据
config.example.json     默认公开配置模板
```

## 隐私和数据

Paper Monitor 只在本地保存运行数据。默认情况下，它不会上传你的检索历史、匹配论文或配置文件。

公开仓库中不会包含：

- 个人 `config.json`
- SQLite 数据库
- 日志文件
- Crossref 缓存
- 本机构建目录

## 常见问题

### 为什么没有收到通知？

请检查：

- Windows 通知权限是否允许 Paper Monitor。
- 是否开启了专注模式。
- 是否确实检索到了新的匹配文章。
- 已经出现过的文章不会重复通知。

### 为什么某些文章的日期只有年月，或与期刊网页不同？

不同数据来源提供的发表日期字段和精度可能不同。Paper Monitor 会保留来源的原始日期精度；来源只提供年月时，不会虚构具体日期。首页时间线使用首次检测时间进行排序和分组，并将 `Detected` 与来源提供的 `Published` 日期分别显示，因此刚发现的论文不会因为发表日期精度较低而被挤出第一页。

### 可以换成其他研究方向吗？

可以。用户可以在设置里修改检索词、排除词和查询语句，也可以从 300 本跨学科目录中按类别筛选期刊。默认检索词偏向固态电池方向，但核心逻辑可以用于其他文献监控任务。
