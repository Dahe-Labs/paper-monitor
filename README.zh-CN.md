...
当前默认检索词主要面向全固态电池、固态电解质、电极材料和锂金属负极，但候选库已扩展为 300 本跨学科期刊，覆盖 AI、计算机、工程、医学、生命科学、物理、化学、材料、环境、数学和社会科学。用户可以在设置里改成自己的关键词、期刊范围和检索方向。
...
Windows 版关闭窗口后会结束本次界面进程并释放 WebView、Python 和本地服务占用，只保留一个不包含网络检索、数据库或页面渲染逻辑的轻量原生 C 托盘。开启“后台监控”后，由 Windows 任务计划程序在检索到期时启动一个短时刷新进程；完成检索、存储和通知后进程立即退出，因此两次检索之间不会常驻 Python、WebView 或本地 HTTP 服务。
