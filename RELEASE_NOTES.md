...
- The repository, CI jobs, packaging scripts, and downloadable assets now target Windows only.
...
- Consolidated icon generation around `windows/assets/AppIconSource.png`; the Windows app, pywebview window, native tray, shortcuts, Toast identity, and installer all consume its generated ICO.
- Removed dead icon wrappers, obsolete installer code, and duplicate onedir resources without deleting compatibility migrations or user data.
...
- Standardized GitHub Release asset versioning for Windows downloads.
- Added a Windows GitHub Actions workflow that generates release assets with the visible application version.

## Notes

- The Windows artifact is unsigned unless a trusted Authenticode certificate is configured.
